...
void I_GetEvent(void)
{
    ...
    // TODO consume input events from the wpilib event queue
...
void I_StartTic(void)
{
    ...
    // TODO - call I_GetEvent() while there are stored events to consume from wpilib
...
void I_FinishUpdate(void)
{
    ...
    // TODO - call a display update hook
    ...
    snprintf(name, sizeof(name), "bins/%05u.bin", frame_count++);
    fd = fopen(name, "wb");
    if (fd != NULL)
    {
...
// I_ReadPalette
//
void I_ReadPalette(byte *palette)
{
    memcpy(palette, cur_palette, 256 * 3);
}
...
{
    memcpy(cur_palette, palette, 256 * 3);
}
